...
The scope of Boot and Leiningen is larger, providing development-time workflows and extension mechanisms.

Boot and Leiningen can also produce artifacts, but Meyvn leverages Maven plugins to provide additional services.
And Meyvn’s interface with the Clojurescript compiler is unmediated (unlike Boot and Leiningen).

In other words, there is no competition, only complementary options.

** No Lip Service

The NoLipService library is used to gain visibility into the commercial usage of Meyvn.
Companies who use Meyvn may be approached with proposals to acquire a commercial license.

Don’t pay lip service to OSS, support it actively.
...
commercial features, and devoid of ~NoLipService~. Finally, licensees
get access to email support.
