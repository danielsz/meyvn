...
1. Commercial and OSS version have parity of features
2. Commercial version has more features than OSS 
...
*Pros*: The community benefits
*Cons*: Tracking

The second model doesn’t need tracking, because the distinction between a free and a pro version with many more features is by itself an incentive to buy the “better version”. 

*Pros*: No tracking
*Cons*: The community loses

The current model I am putting to test is the first model (with opt-in tracking). When you opt-in, Meyvn will send the POM’s group ID and success result of each execution back to an analytics server. When you opt-out, the program quits. At this stage, I am interested in users who are supportive and for whom finding ways to do sustainable OSS is important and not mere lip service.
