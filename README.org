...
- Find the prefix for the Maven installation on your system.
...
The takeaway for Meyvn is that building on top of the Maven ecosystem is rewarding. It is a huge ecosystem, well documented and extremely mature. A lot of functionality just sits there, waiting to be tapped by our tooling (continuous integration, generated documentation, testing, reporting, etc.)
...
Boot and Leiningen can also produce artifacts, but their scope is larger, providing development-time workflows and extension mechanisms.

Meyvn leverages Maven to do its thing. And Meyvn’s interface with the Clojurescript compiler is unmediated (unlike Boot and Leiningen).

In other words, there is no competition, only complementary options.

** No Lip Service

The NoLipService library is used to gain visibility into the enterprise usage of Meyvn. Companies who use Meyvn may be approached with proposals to acquire a commercial license.

Writing and maintaining Open Source Software takes time and effort. Be a mensch. [[https://www.patreon.com/danielsz][Patronize]] my tuchus.
...
commercial features, and devoid of NoLipService’s tracking of group
id associated with builds. 
